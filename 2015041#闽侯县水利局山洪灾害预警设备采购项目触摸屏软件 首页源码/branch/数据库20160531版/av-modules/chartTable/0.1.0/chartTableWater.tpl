...
<div ms-class="row chartTable-detail" ms-visible="isShowDetail">
    <label class="detail-item-col5" ms-repeat="currentData">{{currentDK[$key]}} : {{$val}}<span ms-if-loop="$index%2==0">米</span><span ms-if-loop="$index%2!=0">万方</span></label>
</div>
<div ms-class="row chartTable-table" >
    ...
    <div class="col-xs-12 chartTable-content" ms-class-1="chartTable-content-water">
        <table class="table table-bordered">
            <col ms-css-width="{{100/(tableHead.length)}}%" ms-repeat="tableHead"/>
