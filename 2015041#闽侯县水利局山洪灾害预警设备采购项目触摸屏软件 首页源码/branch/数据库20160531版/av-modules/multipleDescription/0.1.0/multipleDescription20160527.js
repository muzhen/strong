    ...
    var table = {
        ...
        getdata:function(options){
            ...
            $.getJSON(options.data.a3 ,par_a3 , function (result) {
             ...
             //console.log("等值面图src"+options.tableVm.rainChart)
            });
            ...
            $.getJSON(options.data.a5,  function (result) {
                ...
                for( var i= 0;i < waterNum;i++){
                    var flagnum = options.tableVm.waterData[i].flag ;
                    if(flagnum){
                        num++;
                    }else{
                        num;
            ...
            $.getJSON(options.data.a6,  function (result) {
                ...
                for( var i= 0;i < reservoirNum;i++){
                    var flagnum = options.tableVm.reservoirData[i].flag ;
                    if(flagnum){
                        num++;
                    }else{
                        num;
...
function realTime() {
   ...
   //  var   currentTimeHours = 7,//用于和8点做比较
     ...
     //  eicontrastTime = new Date().setHours(8,0,0).toLocaleTimeString();
       eicontrastTime =currentTime.setHours(8,0,0),
         formatToday = GetCurrentTime(true, 0, currentTime),//格式化时间，今天
        formatYesterday = GetCurrentTime(true, -1, currentTime);//格式化时间，前一天
    var localeDate = currentTime.toLocaleDateString();     //获取当前日期 2016/5/26
    var localesDate = currentTime.toLocaleString( );    //获取当前日期 2016/5/26 上午8:33:22
    var localessDate = currentTime.toLocaleTimeString();    //获取当前日期 上午8:34:13

    console.log("时分s："+ localessDate );
    if (contrastTime <= localessDate) {
        ...
        etimeChart =  etime.replace("T", " ");//雨量 等值面图 2016-05-25 15:16:28
     //
     //console.log("大于8点："+ currentTime );
        console.log("当前8点2："+ localessDate );
        console.log(" 8点："+ eicontrastTime );
    }
    var obj={
        key:key_led,
        addvcd:addvcd,
