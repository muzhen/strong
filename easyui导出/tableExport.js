	...
	view.tableExport = function(tableId, filename, type){
		...
		var toOffice = function(){
			...
			var office = '',
				...
				mapb = [['<th>', '</th>'],['<td>', '</td>']],
				//maph = [['<div class="thead"><div>', '</div></div>'], ['<tbody><tr>', '</tr></tbody>'], ['<tr>', '</tr>']],
				//mapb = [['<th>', '</th>'],['<td>', '</td>']],
				 flag = +!table.tHead,
				//flag = +!table.children().class() tHead,
				com = 1 - flag;

			for(var i=0, row; row = table.rows[i]; i++){//第一行开始， i第几行
				flag = i > com ? 2 : flag;
				office += maph[flag][0];
				for(var j =0, col; col = row.cells[j]; j++){//j 一行几列
